...
namespace deexcitation {
    ...
    namespace Chatterjee {
        ...
        __device__ float computeProbabilityShared(float k) {
            ...
            erec = fmaxf(erec, 0.f);
            float xs      = crossSectionShared(erec);
        ...
        __device__ float computeProbability(int channel, float k, float a0, float a1, float delta0, float delta1) {
            ...
            float erec    = (m * (k + em) - em * em) / m_res - em;
            erec = fmaxf(erec, 0.f);
            float xs      = crossSection(channel, erec);
        ...
        __device__ float emitParticleEnergy(curandState* state, CHANNEL channel) {
            ...
            setParameters(channel, elim, cache_zaev[threadIdx.x].y);
    ...
    namespace Kalbach {
        ...
        __device__ float computeProbability(int channel, float k, float a0, float a1, float delta0, float delta1, float signor) {
            ...
            float erec    = (m * (k + em) - em * em) / m_res - em;
            erec = fmaxf(erec, 0.f);
            float xs      = crossSection(channel, erec, signor);
            float prob    = (float)PROJ_S[channel] * PROJ_M[channel] * PROB_COEFF 
                * expf(2.f * (sqrtf(a1 * e1) - sqrtf(a0 * e0))) * k * xs;
            if (exc_res < 0.f)
                prob = 0.f;
        ...
        __device__ float emitParticleEnergy(curandState* state, CHANNEL channel) {
            ...
            mcutil::cache_univ[CUDA_WARP_SIZE + 5 * blockDim.x + threadIdx.x] 
                = channel == CHANNEL::CHANNEL_NEUTRON ? fminf(4.f, 100.f / (float)cache_zaev[threadIdx.x].y) : elim;  // Coulomb barrier

            // Kalbach parameters
            setParameters(channel, cache_zaev[threadIdx.x].y);
            ...
            float signor = 1.f;
            a = cache_zaev[threadIdx.x].y;
            if (channel == CHANNEL::CHANNEL_NEUTRON) {
                if (a < 40)
                    signor = 0.7f + (float)a * 0.0075f;
                else if (a > 210)
                    signor = 1.f + (float)(a - 210) * 0.004f;
            }
            else if (channel == CHANNEL::CHANNEL_PROTON) {
                if (a <= 60)
                    signor = 0.92f;
                else if (a < 100)
                    signor = 0.8f + (float)a * 0.002f;
            }
            ...
            do {
                x = (eke_max - eke_min) * (1.f - curand_uniform(state));
                w = computeProbability(channel, x + eke_min, a0, a1, delta0, delta1, signor);
            } while (wmax * curand_uniform(state) > w);
